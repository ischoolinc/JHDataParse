...
namespace JHDataParse.UIForm
{
    public partial class frmDeleteSHSemsSubjectScore : FISCA.Presentation.Controls.BaseForm
    {
        ...
        // 所有科目名稱
        List<string> AllSubjList;

        // 需要刪除科目名稱
        List<string> DelSubjList;
        ...
        public frmDeleteSHSemsSubjectScore()
        {
            ...
            AllSubjList = new List<string>();
            DelSubjList = new List<string>();
        ...
        private void _bgWorkerDelData_RunWorkerCompleted(object sender, RunWorkerCompletedEventArgs e)
        {
            ...
            MsgBox.Show("已完成刪除，若刪除該科目會影響學期成績，請重新計算學期成績。");
            // 重新載入
            ControlEnable(false);
        ...
        private void _bgWorkerDelData_DoWork(object sender, DoWorkEventArgs e)
        {
            ...
            {
                bool isDel = false;
                foreach (string subjName in DelSubjList)
                {
                    if (ss.ScoreInfoXML.Element("SemesterSubjectScoreInfo") != null)
                        foreach (XElement elm in ss.ScoreInfoXML.Element("SemesterSubjectScoreInfo").Elements("Subject"))
                        {
                            if (elm.Attribute("科目").Value == subjName)
                            {
                                // log 科目
                                if (!LogSubjDataDict.ContainsKey(ss.StudentID))
                                    LogSubjDataDict.Add(ss.StudentID, new List<string>());

                                LogSubjDataDict[ss.StudentID].Add(string.Format("科目名稱：{0} ", subjName));
            ...
            if (DelSemsScoreList.Count > 0)
            {
                ...
                foreach (SemsScoreInfo ss in DelSemsScoreList)
                {
                    ...
                    {
                        innerText.AppendLine(elm.ToString());
                    }
        ...
        {
            dgData.Rows.Clear();
            foreach (string key in AllSubjList)
            {
                int rowIdx = dgData.Rows.Add();
                dgData.Rows[rowIdx].Tag = key;
                dgData.Rows[rowIdx].Cells["科目名稱"].Value = key;
            }
